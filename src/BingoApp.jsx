...
const generateRandomNumbers = () => {
  ...
  return numbers.slice(0, 24); // 24 because the middle is FREE
};
...
export default function BingoApp() {
  ...
  return (
      <div className="app">
        ...
        <p className="subtitle">Tap a number and win with a row, column or diagonal!</p>
        <div className="board">
          {board.map((row, rowIdx) => (
              <div key={rowIdx} className="row">
                {row.map((cell, colIdx) => (
                    <div
                        key={colIdx}
                        className={`cell ${cell.marked ? 'marked' : ''} ${cell.word === 'FREE' ? 'free' : ''}`}
